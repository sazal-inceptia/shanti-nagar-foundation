...
@section('content')
                ...
                <div class="card table-card">
                    <div class="card-header table-header">
                        <div class="title-with-breadcrumb">
                            <div class="table-title">Expenses &amp; Vouchers</div>
                        ...
                        <a href="{{ route('admin.expenses.create') }}" class="add-new">
                            Record Expense Voucher <i class="ms-1 ri-add-circle-line"></i>
                        </a>
                    </div>
                    ...
                    <div class="card-body border-bottom" style="background-color: #f8fafc; padding: 14px 20px;">
                        <div class="row g-2 align-items-end">
                            <div class="col-md-3 col-sm-6">
                                ...
                                <select id="filter_category" class="form-select form-select-sm custom-input" style="height: 32px; font-size: 13px;">
                                    <option value="">All Expense Categories</option>
                                    @foreach($categories as $catVal => $catLabel)
                                        <option value="{{ $catVal }}" {{ request('expense_category') == $catVal ? 'selected' : '' }}>
                                            {{ $catLabel }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div class="col-md-3 col-sm-6">
                                <label class="form-label mb-1 text-muted"
                                    style="font-size: 11.5px; font-weight: 600; text-transform: uppercase; letter-spacing: 0.05em;">Project Allocation</label>
                                <select id="filter_project" class="form-select form-select-sm custom-input" style="height: 32px; font-size: 13px;">
                                    <option value="">All (Projects + General Admin)</option>
                    ...
                    <div class="card-body" style="padding: 20px;">
                        <table class="table dataTable w-100" id="expenses-table" style="min-width: 850px;">
                            <thead>
                                <tr>
                                    <th scope="col" style="width: 45px;">SL</th>
                                    <th scope="col" style="width: 125px;">Voucher #</th>
                                    <th scope="col" style="min-width: 260px;">Title &amp; Vendor</th>
                                    <th scope="col" style="width: 155px;">Category</th>
                                    <th scope="col" style="width: 200px;">Allocated Project</th>
    ...
    <div class="modal fade" id="deleteExpenseModal" tabindex="-1" aria-labelledby="deleteModalTitle" aria-hidden="true">
                ...
                <div class="modal-header" style="background-color: #fee2e2; border-bottom: 1px solid #fca5a5; padding: 16px 20px;">
                    <div class="d-flex align-items-center gap-2">
                        ...
                        <h5 class="modal-title fw-bold text-danger mb-0" id="deleteModalTitle" style="font-size: 16px;">
                            Confirm Voucher Deletion</h5>
                    </div>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <div class="modal-body" style="padding: 20px;">
                    <p class="mb-2" style="font-size: 14px; color: #334155;">
                        Are you sure you want to delete debit voucher <strong id="deleteExpenseTitle" class="text-dark"></strong>?
                    </p>
                    <p class="text-muted mb-0" style="font-size: 12.5px;">
                        This action will soft-delete the transaction and re-calculate related project budget balances.
                    </p>
                </div>
...
@push('custom-script')
    <script type="text/javascript">
        $(document).ready(function () {
            var listUrl = "{{ route('admin.expenses.index') }}";
            ...
            var table = $('#expenses-table').DataTable({
                ...
                ajax: {
                    url: listUrl,
                    data: function (d) {
                        d.expense_category = $('#filter_category').val();
                ...
                columns: [
                    ...
                    { data: 'voucher_info', name: 'voucher_number', orderable: true },
                    { data: 'expense_title', name: 'title', orderable: true },
                    { data: 'category_badge', name: 'expense_category', orderable: true },
                    ...
                    { data: 'formatted_amount', name: 'amount', orderable: true },
                    {
                        ...
                        render: function (data) {
                            ...
                            var showUrl = "{{ url('/admin/expenses') }}/" + id;
                            var editUrl = "{{ url('/admin/expenses') }}/" + id + "/edit";
                            var deleteUrl = "{{ url('/admin/expenses') }}/" + id;

                            var html = '<div class="action-btn justify-content-center">';
                            html += '<a href="' + showUrl + '" class="btn btn-view" title="View Voucher & Details"><i class="ri-eye-line"></i></a>';
                            html += '<a href="' + editUrl + '" class="btn btn-edit" title="Edit Voucher"><i class="ri-edit-line"></i></a>';
                            html += '<button type="button" class="btn btn-delete btn-delete-modal" data-id="' + id + '" data-title="' + name + '" data-url="' + deleteUrl + '" title="Delete Voucher"><i class="ri-delete-bin-line"></i></button>';
                            html += '</div>';
                            return html;
                        }
                    }
            ...
            $('#filter_category, #filter_project, #filter_method').on('change', function () {
                table.draw();
            });
            ...
            $(document).on('click', '.btn-delete-modal', function () {
                ...
                $('#deleteExpenseForm').attr('action', url);
