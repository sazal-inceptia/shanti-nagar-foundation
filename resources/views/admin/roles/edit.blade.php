...
@section('content')
    <div class="container-fluid my-3">
        <form action="{{ route('role.update', ['id' => $role->id]) }}" method="POST" id="editRoleForm">
            @csrf
            ...
            <div class="card table-card mb-4 border-0 shadow-sm" style="border-radius: 10px;">
                <div class="card-header table-header d-flex align-items-center justify-content-between p-3"
                    ...
                    <div class="title-with-breadcrumb">
                        <div class="table-title fw-bold" style="font-size: 16px; color: #0f172a;">
                            Edit Role: <span
                                class="text-primary">{{ ucwords(str_replace(['-', '_'], ' ', $role->name)) }}</span>
                        </div>
                    ...
                    <div class="row g-3">
                        <div class="col-md-6 col-12">
                            ...
                            <input type="text" class="form-control custom-input @error('name') is-invalid @enderror"
                                ...
                                style="height: 40px; font-size: 13.5px;" {{ $role->name === 'superadmin' ? 'readonly' : '' }}>
                            @error('name')
            ...
            <div class="card table-card mb-4 border-0 shadow-sm" style="border-radius: 10px;">
                                        ...
                                        @foreach ($modulePermissions as $value)
                                            <div class="form-check mb-0 d-flex align-items-center">
                                                <input type="checkbox" name="permission[]"
                                                    ...
                                                    value="{{ $value->id }}" data-module="{{ $module->module }}" {{ (is_array(old('permission')) ? in_array($value->id, old('permission')) : in_array($value->id, $rolePermissions)) ? 'checked' : '' }}>
                ...
                <div class="card-footer bg-white p-3 d-flex align-items-center justify-content-end gap-2"
                    ...
                    <button type="submit" class="btn btn-primary submit-button px-4"
                        ...
                        <i class="ri-check-line me-1"></i> Update Role
...
@push('custom-script')
    <script>
        $(document).ready(function () {
            ...
            // Initialize all module cards on page load (calculates active permissions from database)
